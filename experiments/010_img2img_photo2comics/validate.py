...
from src.img2img import IdentityPairedAugment, PairedImageDataset, build_model_from_ckpt
...
def main():
    ...
    model, sd, train_cfg = build_model_from_ckpt(args.checkpoint, device, use_ema=args.use_ema)
    color_space = train_cfg.get("color_space", "srgb")
    ckpt = torch.load(args.checkpoint, map_location=device)  # re-load for method/flow config
